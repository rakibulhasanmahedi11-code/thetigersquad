  ...
  return (
    <section id="players" className="py-16 bg-background">
      <div className="container mx-auto px-4">
        <h2 className="font-heading text-4xl md:text-5xl font-bold text-foreground mb-2">PLAYERS</h2>
        <p className="text-muted-foreground text-sm mb-8 max-w-md">Browse players from Main Team, Academy Team & Youth Team.</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {players.map((player: any) => (
                  <div key={player.id} onClick={() => navigate(`/player/${player.id}`)}
                    ...
                    <h3 className="font-heading text-sm font-bold text-foreground text-center">{player.name}</h3>
                    <p className="text-xs text-muted-foreground text-center mt-1">Age: {calcAge(player.date_of_birth, player.age)}</p>
                    <p className="text-xs text-primary text-center mt-1 font-heading tracking-wider">{teamDisplayName[player.team]}</p>
                  </div>
                ))}
              </div>
            )}
